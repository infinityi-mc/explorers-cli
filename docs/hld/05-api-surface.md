...
1. **Minecraft Command Executor (`run_command`)**
   - _Arguments_: `{ command: string }`
   - _Auth Rule_: The command must match the agent's allowlist prefix tokens (FR-TOOL-004).
   - _Output_: Plain-text stdout return from the server process.
2. **Filesystem Reader (`read_file`)**
   - _Arguments_: `{ path: string }`
   - _Auth Rule_: Canonicalized path must resolve within the target `server.path` (FR-TOOL-008).
   - _Output_: Verbatim file content.
3. **Filesystem Writer (`write_file`)**
   - _Arguments_: `{ path: string, content: string }`
   - _Auth Rule_: Path must reside inside the sandbox, cannot be an NBT file, and cannot move/delete files when the server is `RUNNING` (FR-TOOL-006).
   - _Output_: Write confirmation status.
