...
export async function loadRuntimeConfig(
  ...
  if (!isRecord(yaml.value.value)) {
    return {
      ok: false,
      diagnostics: [
        configIssue("config.yaml", "Config root must be a YAML object."),
      ],
    };
  }
...
function normalizeRuntimeConfig(
  ...
  if (raw.schemaVersion !== 1) {
    issues.push(
      configIssue("schemaVersion", "Only schemaVersion 1 is supported."),
    );
  }
  ...
  if (!ISO_DURATION_PATTERN.test(raw.sessionRetention)) {
    issues.push(
      configIssue(
        "sessionRetention",
        "Must be an ISO 8601 duration, e.g. P30D.",
      ),
    );
  }
  if (raw.logging.rotationBytes < 1) {
    issues.push(
      configIssue("logging.rotationBytes", "Must be at least 1 byte."),
    );
  }
...
function normalizeServers(
  ...
  for (const entry of entries) {
    ...
    const jarFile = stringField(
      entry.value,
      "jarFile",
      `${path}.jarFile`,
      issues,
    );
    const javaPath = stringField(
      entry.value,
      "javaPath",
      `${path}.javaPath`,
      issues,
    );
    const serverPort = numberField(
      entry.value,
      "serverPort",
      `${path}.serverPort`,
      issues,
    ...
    if (name !== undefined) {
      if (names.has(name))
        issues.push(
          configIssue(`${path}.name`, "Server names must be unique."),
        );
      names.add(name);
    }
...
function normalizeProviders(
  ...
  for (const entry of configEntries(value, "providers", "name", issues)) {
    ...
    const baseUrl = optionalStringField(
      entry.value,
      "baseUrl",
      `${path}.baseUrl`,
      issues,
...
function normalizeAgents(
  ...
  for (const entry of configEntries(value, "agents", "id", issues)) {
    ...
    const systemPrompt = stringField(
      entry.value,
      "systemPrompt",
      `${path}.systemPrompt`,
      issues,
    );
    const tools = stringArrayField(
      entry.value,
      "tools",
      `${path}.tools`,
      issues,
    ...
    if (alias !== undefined) {
      ...
      if (aliases.has(lowerAlias))
        issues.push(
          configIssue(`${path}.alias`, "Agent aliases must be unique."),
        );
      aliases.add(lowerAlias);
    }
    if (provider !== undefined && providers[provider] === undefined) {
      issues.push(
        configIssue(`${path}.provider`, `Unknown provider "${provider}".`),
      );
    }
...
function normalizePermissions(
  ...
  if (!isRecord(value)) {
    issues.push(
      configIssue("permissions", "Must be an object keyed by server id."),
    );
    return out;
  }
  ...
  for (const [serverId, serverPermissions] of Object.entries(value)) {
    ...
    if (!ID_PATTERN.test(serverId)) {
      issues.push(
        configIssue(
          path,
          "Server permission keys must match ^[a-zA-Z0-9_-]{1,32}$.",
        ),
      );
      continue;
    }
    ...
    if (
      !isRecord(serverPermissions) ||
      !Array.isArray(serverPermissions.players)
    ) {
      issues.push(
        configIssue(`${path}.players`, "Must be an array of players."),
      );
      continue;
    }
    ...
    for (let i = 0; i < serverPermissions.players.length; i++) {
      ...
      const playerAgents = stringArrayField(
        player,
        "agents",
        `${playerPath}.agents`,
        issues,
      );
      const teamPrefix = optionalStringField(
        player,
        "teamPrefix",
        `${playerPath}.teamPrefix`,
        issues,
      );
      const teamSuffix = optionalStringField(
        player,
        "teamSuffix",
        `${playerPath}.teamSuffix`,
        issues,
      );
      const inGameAdmin = booleanField(
        player,
        "inGameAdmin",
        `${playerPath}.inGameAdmin`,
        issues,
        false,
      );
      for (const agentId of playerAgents ?? []) {
        if (agents[agentId] === undefined) {
          issues.push(
            configIssue(`${playerPath}.agents`, `Unknown agent "${agentId}".`),
          );
        }
      }
      if (
        name !== undefined &&
        playerAgents !== undefined &&
        inGameAdmin !== undefined
      ) {
        players.push({
          name,
...
function configEntries(
  ...
  if (Array.isArray(value)) {
    return value.flatMap((item, index) =>
      entryFromValue(String(index), `${path}.${index}`, item, issues),
    );
  }
  if (isRecord(value)) {
...
function namedField(
  ...
  if (name !== undefined && entry.key !== name && !/^\d+$/.test(entry.key)) {
    issues.push(
      configIssue(
        `${path}.${key}`,
        `Record key "${entry.key}" must match ${key} "${name}".`,
      ),
    );
  }
  return name;
...
function numberField(
  ...
  if (!Number.isInteger(parsed) || parsed < range.min || parsed > range.max) {
    issues.push(
      configIssue(
        path,
        `Must be an integer from ${range.min} to ${range.max}.`,
      ),
    );
    return undefined;
  }
...
function enumField<T extends string>(
  ...
  issues.push(
    configIssue(path, `Must be one of: ${[...variants].join(", ")}.`),
  );
  return undefined;
}
...
function secretEnvField(
  ...
  if (match === null) {
    issues.push(
      configIssue(
        path,
        "Must be an environment reference like ${OPENAI_API_KEY}.",
      ),
    );
    return undefined;
  }
  ...
  if (value === undefined || value.length === 0) {
    issues.push(
      configIssue(path, `Environment variable ${envName} is not set.`),
    );
    return undefined;
  }
  return value;
